...
router.get(`/`,async (req,res)=>{
    const orderList = await Order.find().populate('user','name').sort({'dateOrdred' : -1});
    if(!orderList){
...
router.post(`/`,async (req,res)=>{
    ...
    let order = new Order({
        ...
        shippingAdresse1: req.body.shippingAdresse1,
        shippingAdresse2:req.body.shippingAdresse2,
        city:req.body.city,
        zip:req.body.zip,
    ...
    if(!order){
        return res.status(400).send('Ordre cannot be created')
    }
